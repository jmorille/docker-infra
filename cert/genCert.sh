...
function createCertificateTls {
 ...
 openssl genrsa -passout pass:$CERT_PASS -des3 -out $PRIV_DIR/$CERT_FILENAME.key $NUMBITS

 # Enregistrez le fichier $PRIV_DIR/$CERT_FILENAME.key et le mot de passe éventuellement défini en lieu sûr.
 # TODO

 # Vous pouvez afficher les détails de cette clé privée RSA à l'aide de la commande :
 # openssl rsa -passin pass:$CERT_PASS -noout -text -in $PRIV_DIR/$CERT_FILENAME.key
 ...
 openssl req -passin pass:$CERT_PASS -new -key $PRIV_DIR/$CERT_FILENAME.key -out $PUB_DIR/$CERT_FILENAME.csr  -subj $CERT_SUBJ
...
function printCsr {
 ...
 openssl req -passin pass:$CERT_PASS -noout -text -in $PUB_DIR/$CERT_FILENAME.csr
}


function signCertificateTlsWithCa {
  # La commande qui signe la demande de certificat est la suivante : ==>  CRT = CSR + CA sign
  openssl x509 -passin pass:$CA_PASS -req -in $PUB_DIR/$CERT_FILENAME.csr -out $PUB_DIR/$CERT_FILENAME.crt -CA $PUB_DIR/ca.crt -CAkey $PRIV_DIR/ca.key -CAcreateserial -CAserial $PRIV_DIR/ca.srl
...
function printCrt {
  ...
  openssl x509 -noout -text -in $PUB_DIR/$CERT_FILENAME.crt
}
...
function unpassswdCertificateTlsKey {
 ...
 openssl rsa  -passin pass:$CERT_PASS  -in $PRIV_DIR/$CERT_FILENAME.key -out $PUB_DIR/$CERT_FILENAME-nopasswd.key
}
...
function createNewKeystorePKCS12 {
 # -->  server.p12
   ...
   openssl pkcs12 -passin pass:$CERT_PASS -passout pass:$KEYSTORE_PK12_PASS  -export -in $PUB_DIR/$CERT_FILENAME.crt -inkey $PRIV_DIR/$CERT_FILENAME.key -out $FILE_KEYSTORE_PKCS12 -name tomcat -CAfile $PUB_DIR/allcacerts.crt -caname root -chain
 }
...
function createNewKeystoreJKS {
   ...
   #keytool -certreq -keyalg RSA -alias server -file $PUB_DIR/$CERT_FILENAME.csr -keystore server-keystore.jks -storepass $CA_PASS01 -keypass $CA_PASS01
...
function importKeystoreJKSCertificates { 
 ...
 # keytool -import -alias tomcat -keystore $FILE_KEYSTORE_JKS -storepass $KEYSTORE_JKS_PASS -trustcacerts -noprompt -file  $PUB_DIR/$CERT_FILENAME.crt
...
function setup {
  ...
  # Create Server Certificate ==>  $PUB_DIR/$CERT_FILENAME.csr
  createCertificateTls || exit 1

  # Sign Server Certificate With CA  ==> $PUB_DIR/$CERT_FILENAME.crt
